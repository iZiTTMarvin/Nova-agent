...
import { StreamingFileCard, type StreamingFileCardProps } from '../../../src/renderer/features/chat/StreamingFileCard'
...
function renderCard(props: StreamingFileCardProps) {
...
describe('StreamingFileCard Step 2 优化', () => {
  ...
  it('running 阶段不调用 highlightLine（纯文本展示）', () => {
    ...
    const lineTexts = renderer.container.querySelectorAll<HTMLElement>('.streaming-card__line-text')
    expect(lineTexts.length).toBe(2)
    expect(lineTexts[0]?.textContent).toBe('body { color: red; }')
    expect(lineTexts[1]?.textContent).toBe('.foo { padding: 1px; }')
  ...
  it('接 argumentsRaw 时正确解析 path 和 content（仅 partial JSON）', () => {
    ...
    const filename = renderer.container.querySelector<HTMLElement>('.streaming-card__filename')
    expect(filename?.textContent).toBe('src/foo.css')

    const lineText = renderer.container.querySelector<HTMLElement>('.streaming-card__line-text')
    expect(lineText?.textContent).toBe('body { color: red;')
  ...
  it('向后兼容：仅传 args 时仍能正确显示', () => {
    ...
    const filename = renderer.container.querySelector<HTMLElement>('.streaming-card__filename')
    expect(filename?.textContent).toBe('src/legacy.css')

    const lineText = renderer.container.querySelector<HTMLElement>('.streaming-card__line-text')
    expect(lineText?.textContent).toBe('a { color: blue; }')
  ...
  it('edit 工具的 partial raw（filePath + newText）正确显示', () => {
    ...
    const filename = renderer.container.querySelector<HTMLElement>('.streaming-card__filename')
    expect(filename?.textContent).toBe('src/foo.ts')

    const lineText = renderer.container.querySelector<HTMLElement>('.streaming-card__line-text')
    expect(lineText?.textContent).toBe('// new')
  ...
  it('接完整闭合的 argumentsRaw：能解析 path/content 并展示', () => {
    ...
    const lineText = renderer.container.querySelector<HTMLElement>('.streaming-card__line-text')
    expect(lineText?.textContent).toBe('a = 1;')
