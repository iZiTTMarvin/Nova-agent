...
- **feat(ui)**: 会话导航体验对齐主流 Agent 桌面端：置顶分区 + 标题栏面包屑与会话快捷操作
  - 侧边栏新增置顶分区：会话菜单可把常用会话钉到顶部独立分区（带图钉标识），置顶状态随会话落盘，重开应用不丢；置顶只改展示分区，不打乱各项目的最近活跃排序
  - 对话区顶部新增「工作区 / 当前会话」路径面包屑（文件夹图标 + 路径式胶囊），点击展开当前工作区下的会话列表快速切换；焦点在子代理会话时显示其父会话，与侧边栏高亮规则一致
  - 标题栏右侧新增当前会话「⋯」菜单：置顶/取消置顶、删除当前会话，不用回侧边栏找行
