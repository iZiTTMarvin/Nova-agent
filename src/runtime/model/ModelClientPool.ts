...
import { resolveRequestReasoningEffort } from './reasoningDialect'
/**
...
export class ModelClientPool implements ModelClient {
  ...
  measureRequest(messages: ChatMessage[], tools?: ToolDefinition[], options?: ChatOptions): RequestBudgetMeasurement {
    ...
    const reasoningEffort = resolveRequestReasoningEffort(
      slot.config.modelId,
      slot.config.reasoningEffort,
      options?.reasoningEffort,
      slot.config.baseUrl
    )
    const route = resolveRouteIdentity({ ...slot.config, reasoningEffort })
    return measureRequestBudget({ model: slot.config.modelId, messages, tools }, route.routeId, resolveContextWindow(slot.config.modelId, slot.config.contextWindow))
  }
