...
function assertTrustedSender(event: IpcMainInvokeEvent, channel: string): void {
  ...
  if (!trusted) {
    console.error(`[secureIpc] 拒绝 IPC channel=${channel}：主窗口未就绪`)
    throw new Error('IPC 调用被拒绝：主窗口未就绪')
  }

  if (event.sender !== trusted) {
    console.error(`[secureIpc] 拒绝 IPC channel=${channel}：非主窗口 webContents`)
    throw new Error('IPC 调用被拒绝：来源不可信')
  }

  if (event.senderFrame !== event.sender.mainFrame) {
    console.error(`[secureIpc] 拒绝 IPC channel=${channel}：非主 frame`)
