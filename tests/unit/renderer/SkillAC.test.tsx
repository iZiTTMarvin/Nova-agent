...
describe('SkillAC 浮层关闭逻辑（参数阶段不拦截 Enter）', () => {
  it('输入纯 slash 命令（无空白）时，Enter 选中候选并拦截', async () => {
    const { onKeyDown, onSelect, renderer } = await renderSkillAC('/frontend')
  ...
  it('选中后输入框出现尾随空格时，Enter 不再拦截（回归发送路径）', async () => {
    const { onKeyDown, onSelect, renderer } = await renderSkillAC('/frontend-design ')
    const e = keyEvent('Enter')
  ...
  it('选中后在 slash 命令后追加参数时，Enter 不拦截、不重置输入框', async () => {
    ...
    const { onKeyDown, onSelect, renderer } = await renderSkillAC('/frontend-design 做一个登录页')
    const e = keyEvent('Enter')
