 ...
 * （其自身会校验权威运行快照并置 sendInFlight，与手动发送行为一致）。
 *
...
export async function dispatchNextPendingMessage(api: ChatStoreApi): Promise<void> {
  const { pendingUserMessages, sendMessage, sendInFlight, currentSessionId } = api.getState()
  ...
  try {
    ...
    if (!sent && api.getState().currentSessionId === currentSessionId) {
    ...
    if (api.getState().currentSessionId === currentSessionId) {
      api.setState(state => ({ pendingUserMessages: [next, ...state.pendingUserMessages] }))
    }
  }
}
