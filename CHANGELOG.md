...
- **模型选择更简洁**：输入区直接列出可切换模型；思考强度只展示当前模型真实支持的档位，默认档位直接落在具体选项上，MiniMax 这类没有可区分档位的模型不再出现无效强度菜单。
- **模型切换更跟手**：回复进行中也能切换当前会话的模型与思考强度，正在生成的回复不受影响，下一条消息起改用新选择；切换本身不再重读会话记录与全部会话列表，点选立即生效。
- **长会话归档更懂算账**：大段工具结果是否归档改为按服务商缓存价格算回本：低压力时只在少携带几轮就能抵回缓存重建成本时才归档，且优先选最靠后的断点，日常会话几乎不再打断缓存前缀；上下文逼近压缩线时改为「能省够就先无损归档」，尽量把有损的整段压缩往后推。
- **长会话压缩更省也更稳**：生成压缩摘要的请求与正常对话共用同一段缓存前缀，不再每次压缩都把整段历史按全价重算一遍；摘要偶发格式不合法时不再让整个任务失败，本轮先照常继续，稍后再试。
- **读完的长文件不再反复翻回**：大段工具结果不再看过一轮就换成占位符，只在结果滑出最近工作窗口、且一批能回收足够空间时才归档；刚读完的内容在后续编辑步骤里保持可见，模型请求缓存前缀也只被打断一次。archive_read 单页上限提高到 4 万字符，一次可读回中等大小文件。
- **打开长会话更快**：进入会话先展示最近对话，更早的记录按需往上补载；切换会话不再为刷新侧栏而重读全部对话。
- **启动更快**：打开应用时不再为已经结束的任务重读全部执行记录，清理工作放到窗口出现之后。
- **崩溃后未打开的会话也会留下中断回复**：关掉应用时还在跑的对话，下次启动不必先点开该会话，中断内容就已经写进记录。
- **工具流水更顺畅**：连续调用工具时界面不再被频繁落盘卡住。
- **工具进行中更稳**：工具跑起来时状态不再一条条打断界面。
- **大仓库里只读命令更顺畅**：查看目录、状态这类不改文件的命令不再先扫一遍全部文件内容，大仓库里不再因此卡住界面；真正改文件的命令仍可回退。
- **设置切换即时**：切换设置分区不再整页重载，回到刚看过的分区无需再等加载。
- **提问提交有进度**：提交答案时按钮显示「提交中…」，失败时就地说明原因，提问不会悄悄消失。
- **思考型模型不再空白停住**：模型只输出思考、没有正文也没有工具时，会自动再推进一次，而不是留下空白回复。
- **长对话等待回复更跟手**：上下文攒得很长时，每次请求模型前的准备计算不再重复序列化整段对话，等待「思考中」时界面更少出现整段卡顿。
- **多工具长任务更流畅**：一轮任务里调用大量工具时，流式输出期间界面不再整段重算过程时间线，长轮次越到后面越保持顺滑。
- **本会话允许的命令重启后仍有效**：对某条命令选过「本会话允许」后，关掉应用再打开同一个会话不必再批一次；删掉会话后这些允许一并消失，明确拒绝和高风险命令仍然优先。
- **模型操控内置浏览器更可靠**：各家模型都能正确看到浏览器工具的参数；多填的无关字段不再导致调用被拒，出错时直接告诉模型正确写法，不会原样重试到自动中断；在已有页面跳转不再新开页面，只开了一个页面时读取可省略页面编号；打开、后退、前进、刷新都等页面真正到位后才回报，后退不再失效。
- **本地预览不再误开 Nova 自己的界面**：开发模式下 Nova 界面服务改用不常见的固定端口，不再抢占项目常用的 5173；内置浏览器拒绝打开 Nova 界面占用的端口，并提示换端口，而不是显示崩溃的 Nova 界面。

## 0.1.4 - 2026-09-06

- **长会话交接更可靠**：压缩后保留结构化交接信息，支持回读历史原文，改善上下文预算控制与中断后的恢复。
- **会话记录更完整**：保存完整工具结果与消息事实，改善重新打开会话后的连续性。
- **跨会话记忆更实用**：改进记忆检索与注入，减少对模型请求缓存前缀的干扰。
- **取消与失败处理更明确**：改善取消响应；远端结果未知时停止自动重试，避免重复请求。
- **回滚与阅读体验更稳定**：改善大文件、大仓库下的回滚保护，流式输出时保留用户滚动位置。
- **发布验证更完整**：发布前验证旧版检测、下载、安装与会话保留，发布后核验公开更新链路。

## 0.1.3 - 2026-09-02

- **feat(settings)**: 设置里可以手动检查应用更新
  - 通用设置的「应用更新」区块随时可发起检查，不必再等启动后的自动检查或重启应用
  - 就地展示当前版本与检查结果；发现新版本后可直接下载、查看进度并重启安装
- **feat(subagents)**: 子代理支持带着上下文继续跑
  - 对既有子代理可追加指令继续执行，已读文件与已形成结论全程保留，撞上轮数上限或跑偏后不必从头再来
  - 派遣结果暴露子代理会话标识，父代理可直接点名续跑
  - 同一子代理的每次执行在父会话各有独立活动行，状态互不覆盖；点开可见连续完整历史（含每轮追加指令）
  - 正在执行的子代理拒绝续跑并提示稍后重试；留有未答授权时先提示回答或忽略
  - 续跑沿用既有模型、权限与工具集，无法借续跑提权或换模型；父会话中途停止会级联取消进行中的续跑
- **feat(subagents)**: 子代理轮数按权限分档并接上壁钟超时
  - 子代理默认工具轮数收敛为单一来源：只读 40、可写 80，内置与技能派生的子代理统一跟随
  - 新建子代理表单不再预填轮数，留空即按权限档默认执行
  - 每个子代理派遣带 15 分钟兜底超时，超时后结果如实回传父代理且不占用并发额度
  - 子代理撞上轮数上限、重复失败熔断或空参保护时，回传文本不再包含面向人类的操作指引，改为告知截断事实与重新派遣建议
- **feat(subagents)**: 新增通用子代理并收紧可授予工具边界
  - 新增内置 general-purpose 子代理：支持有界混合读写、检索与命令验证，遵循父权限与 shared 隔离，不能再派遣子代理
  - 统一任务与目录发现提示，优先匹配专业角色再考虑通用角色
  - 预设保存边界新增编排与会话工具拦截，自定义配置不可绕过通用边界
- **feat(settings)**: 优化子 Agent 配置体验
  - 使用两步向导创建子 Agent，不再依赖原生输入框和 JSON
  - 增加独立详情页，支持结构化编辑、复制、启停和分层删除
  - 模型和工具只展示当前实际可用选项
  - 保存失败时保留草稿，并就近展示可操作错误；项目覆盖删除后自动恢复全局配置
- **feat(subagents)**: 子代理配置改用稳定身份与层级化保存
  - 每个子代理拥有创建后不可改的稳定 ID，显示名可随时重命名，改名不再影响项目覆盖、历史会话与显式派遣
  - 保存、启停、删除明确区分全局与项目层级；删除项目覆盖后自动恢复显示同名全局配置，不再连带误删
  - 内置代理在内核边界拒绝写入与删除，不只靠界面隐藏按钮
  - 配置损坏或版本不认识时如实报错并给出处置建议，不再静默显示为「没有配置」，也不会被新保存悄悄清空
  - 中文、特殊字符名称自动生成合法 ID，重名自动加后缀；旧格式配置自动迁移，首次显式保存后固化为新格式
- **perf(chat)**: 大文件工具参数流式预览更顺滑
  - 同一批次内同一工具调用只解析和写入一次，写大文件时 300ms 刷新点不再因重复解析卡住主线程
  - 流式文件卡片、工具轨迹与最终结果保持不变，取消与错误收尾不受影响
- **perf(chat)**: 工具流水期间消息区与等待徽标更稳定
  - 计划审批的投影与订阅收窄为稳定小视图：等价快照不再触发无关消息的重渲染，审批卡只下传给所属会话与消息
  - 消息行的展开回调按消息稳定化，子树的 memo 在消息重渲染时仍保持命中
  - 等待徽标只在等待数量变化时查询主进程，相同结果不产生额外渲染，倒序到达的旧响应不覆盖新结果
- **feat(subagents)**: 子代理支持明确模型路由与有限只读并行
  - 自定义子代理可跟随默认模型或固定到可用模型，并只展示模型明确支持的思考强度
  - 单次派遣可覆盖模型与思考强度；歧义、不可用或能力未知时在创建子会话前明确失败
  - 两到四个独立只读任务可并行执行，容量不足时有界排队，部分失败仍保留其他结果
  - 停止父任务会取消未完成项，单独停止子任务不影响其他项；项目级配置与同 ID 覆盖按当前工作区生效
- **perf(inspector)**: 调整右侧面板宽度时聊天内容不再卡顿
  - 拖动分隔条时，消息阅读区与输入框保持拖动开始时的宽度，收窄的空间自然裁剪，不再逐帧重新排版文字与重测行高
  - 面板内部的审阅 diff 与文件内容在拖动中同样保持稳定，不再随指针反复回流
  - 指针高频移动合并为每个动画帧处理一次，松手时最终宽度精确落定并只保存一次
  - 流式输出期间的自动跟底与手动上滚阅读不受影响

## 0.1.2 - 2026-08-30

- **feat(update)**: 新版本到达时由用户决定何时下载与安装
  - 启动后自动检查公开 GitHub Release，发现更新时在设置旁显示提示，不再后台静默下载
  - 更新弹窗展示版本、发布日期和完整更新日志，并提供下载进度、失败重试与重启安装入口
  - 发布流程自动生成 Windows 安装包、差分文件和更新元数据，版本标签与应用版本不一致时停止发布

## 2026-08-26

- **feat(permissions)**: 权限模式改为跟随会话，并在输入区直接可见
  - 提供「请求批准 / 自动 / 完全访问」三档权限；完全访问启用前会明确确认，运行中与等待授权时不能切换
  - 自动模式放行常规开发操作，高风险命令、网络写入和工作区外访问仍会请求确认；Plan 始终保持只读，显式拒绝规则不会被完全访问绕过
  - 文件、Shell、进程控制与网络按真实副作用分别授权；工作区外路径统一按规范路径校验，符号链接与目录联接不能绕过工作区边界
  - 子代理继承父会话权限，独立的只读上限不再伪装成 Plan 模式；无界面运行必须显式选择权限模式，不再因漏装权限模块而静默放行

## 2026-08-25

- **feat(agent)**: 长命令与交互式程序不再被限时终止，可转入会话持续观察与交互
  - 终端命令运行超过两分钟不再被强制杀掉：自动转入后台会话并返回引用，可继续读输出、写入输入或终止，构建、安装、数据迁移这类长任务不再中途夭折
  - 交互式程序从基本不可用变为可用：Python / Node 等 REPL、登录提示、需要回车确认的向导，都能在进入会话后继续对话式操作
  - 新增「终端会话」工具：读取新输出（输出静默即返回，不空等）、写入输入、中断与终止；观察与终止类动作全模式可用，写入在只读阶段被拦下，会启动交互入口的命令始终逐次确认且不能被白名单放行
  - 会话输出自动剥离控制字符、折叠进度条刷屏；大量输出滚动保留末尾，完整内容落盘可查，临时文件随会话收尾与下次启动清理
  - 每个聊天会话最多 8 个活跃进程、未读输出总量有上限，超限明确提示而不是悄悄丢弃；删除会话、中止任务、子任务结束、退出应用都会收掉所属进程
  - 新增「持久终端会话」设置项（默认开启），关闭后退回原先的限时终止行为

## 2026-08-24

- **feat(chat)**: 工作过程完成后整体折叠，只留最终答复
  - 以最终答复为界：工作中全程展开，可直接看到思考、工具与计划审批；答复输出后整个过程（含思考、工具、问答与计划卡）丝滑折叠，展开「已工作」可回看
  - 完成后显示「已工作 X 分 X 秒」（中止为「已停止 · 工作了 …」），重开会话仍保留时长；进行中计时只在底部追踪器，消息里不再出现「工作中」折叠头
  - 连续探索、写入和命令收成中文摘要，展开后是平铺时间线，不是目录树
  - 计划审批卡归入工作过程，等待批准时保持可见可交互；批准与修改后不再外露残留
  - 忽略计划的决定随审批卡留档：完成后原位翻为灰态「已忽略」，重启回看仍然成立
  - 没有生成总结的轮次显示「已结束，未生成总结」，不再拿过程内容充当结论
- **feat(chat)**: 计划审阅保持在同一个任务中连续完成
  - 计划生成后直接进入任务内审批；批准后沿用当前运行继续实施，修改意见原样回灌并原位更新计划，忽略则正常结束并保留当前计划
  - 计划卡改为紧凑预览，支持复制与右侧完整阅读；关闭完整计划后恢复此前的审阅或文件面板状态
  - 普通 Plan 与 XForge 共用批准、修改和忽略交互，旧的卡片代发消息与单独批准入口已移除

## 2026-08-22

- **fix(code-index)**: 开发启动可靠识别索引数据库的原生运行环境
  - Node 测试留下不同 ABI 的数据库绑定时，启动前直接指出版本差异和恢复命令，不再进入应用后反复报索引打开失败
  - 主进程装配入口不再被功能模块反向加载，IPC 与窗口生命周期只初始化一次
  - 真实 Electron 冷启动、索引构建、查询、重载恢复与工作区切换链路通过
- **fix(chat)**: 计划审阅卡操作加固：失败可回滚重试，历史卡片降级只读
  - 点「执行」后实施指令未能发出时（如 Agent 正忙），自动切回计划模式并保留重试入口，不再卡在默认模式且卡片无按钮可用；回滚失败时明确提示需手动切回
  - 卡片的执行/更正/批准操作前校验所属会话：页面切到其他会话后点击旧卡片会被拒绝并提示，批准与更正指令不再可能发进错误会话
  - 同一计划多轮修订后只有最新卡片提供「执行 / 需要更正」，历史卡片保留内容阅读并提示以最新卡片为准，不再多张可批准卡片并存
  - 测试：失败回滚、会话归属守卫、历史卡只读与最新卡选择等定向用例；typecheck 通过
- **fix(agent)**: 中断的轮次不再残留「永远执行中」的工具记录
  - 轮次被中断或出错结束时，仍在执行中的工具调用在落盘前统一收敛为中断终态；重启后计划卡片显示中断结果，不再永远转圈且无按钮
  - 消息落盘与轮次草稿转正共用同一收口，两条恢复路径都不会把残留带进会话
  - 测试：中断与出错两条终态路径的落盘断言
- **fix(agent)**: 计划批准相关工具措辞与审阅卡契约对齐
  - 模式切换工具不再暗示「计划完成即可切回默认」，改为用户明确批准计划后才使用，与审阅卡承担批准决策的契约一致
  - 提问工具的审批禁令补充豁免：用户要求就更正内容提问时（如「请询问我希望调整哪些部分」）正常提问，避免模型连要改什么都不敢问
  - 工具面基线随文案刷新
- **fix(agent)**: 修复中断与恢复链路的四处卡死
  - 「继续分析」不再把中断的轮次转成无人执行的状态：改为代发一条继续指令开新轮次，发送失败时保留横幅可重试，会话不再永久锁定
  - 切到恰好正在收尾的会话时，以水合完成时刻的最新运行状态为准，不再出现永远转圈、发不出消息的假运行
  - 退出或崩溃时挂起的权限请求在重启对账时统一收敛为已取消，侧边栏不再残留永远清不掉的「等待你处理」
  - 子代理的权限请求现在出现在父会话的子代理活动行上（带「子代理请求权限」标识），可直接允许或拒绝，不再静默卡死子任务
  - 测试：对账收敛、继续语义、水合终态复核、子代理权限路由与锚定等定向用例；typecheck 通过
- **fix(chat)**: 修复八处跨会话与流程错位：状态不再串门，失败不再静默
  - 停止与中断状态按会话归属：在其他会话不再看到或触发属于另一个会话的「正在停止 / 强制终止 / 异常中断」，不再误停错杀
  - 删除正在运行的会话会弹出明确提示，不再点了没反应
  - 轮次出错结束时自动派发已排队的消息；派发失败的消息回到队列原位，不再静默丢失
  - compose 手动「完成计划阶段」自动留下批准记录，计划卡与门禁状态一致，不再出现按钮凭空消失、事后又被门禁拦住
  - compose 批准成功但引导指令未发出时，指令自动放入输入框待确认发送，不再断在半截
  - 重新生成 / 编辑重发的准备窗口内禁用其他消息操作与发送，不再出现分叉竞态导致的消息乱序
  - 分支元数据刷新失败时保留标记并在下轮自动重试，分支翻页器不再永久停在旧值
  - 测试：跨会话归属、删除反馈、队列回队、门禁留痕、分叉守卫、元数据重试等定向用例；typecheck 通过
- **fix(chat)**: 修复八处收尾与边界缺口：草稿不丢、取消不误伤、上限有提示
  - 发送被守卫拦截时（如分叉准备窗口、缺工作区）输入草稿与附件原样保留，不再凭空清空
  - 轮次出错或被取消时同步清理「发送中 / 运行中」绑定，后续发送不再被残留状态挡住
  - 停止任务只清归属本次取消的权限请求，子代理正在等待批准的请求不再被误清
  - 「记住后允许」写入规则失败时中止放行并明确提示，不再留下以为已授权的假象
  - 未重放改动卡的安全禁用与可关闭的横幅解耦：关闭横幅不再解除禁用，评审区与消息流的灰显判定统一为同一来源
  - 切会话时清掉上一会话的分支元刷新标记与灰显，不再串到新会话
  - compose 修复-复审达到上限时回退入口预先禁用并提示「已达审阅修改上限」，不再点了才报错；循环计数随会话水合恢复
  - 会话水合对异常响应增加守卫，不再静默卡在水合中途
  - 测试：草稿保留、取消归属、规则失败中止、灰显同源、上限预禁用等定向用例；修复一处被旁路拉取抢占的既有测试 mock；typecheck 通过
- **fix(chat)**: 中断恢复与强制终止横幅的按钮恢复鼠标可点
  - 「继续分析 / 回滚本轮 / 查看已执行步骤 / 关闭 / 强制终止」此前被 composer 容器的事件穿透设置波及，鼠标点击落空、只能键盘操作；现已恢复正常点击
  - 测试：中断恢复 E2E 改回真实鼠标点击锁定该链路

## 2026-08-21

- **fix(chat)**: 计划审阅交互理顺：审阅卡回到消息流原位，批准决策收归卡片
  - 计划卡片不再钉在整条消息最底部，而是按真实调用顺序插入：过程树之后、收尾说明之前；保存计划后的模式切换行等按发生顺序排在卡片后面
  - 计划保存期间卡片在原位显示「正在生成计划」旋转态，写计划的过程可见
  - 待审阅时卡片右下角改为列表按钮：默认「执行」，下拉可选「需要更正」；点执行自动切回默认模式（输入框旁的 Plan 标识随之消失）并接着实施；点需要更正则停留在计划模式，模型正常追问要改什么后停下等回复，不再用提问面板代替审批
  - 保存计划后模型不再自行切换模式、不再用提问追问"是否开始实施"：计划指令与工具输出明确收尾等待审阅卡选择，提问工具说明新增审批反例护栏
  - 测试：审阅卡执行/更正行为、save_plan 消息流边界顺序、plan 模式指令契约等定向用例更新并通过；typecheck 通过

- **feat(code-index)**: 为 Agent 增加可选的本地代码索引，先定位再阅读
  - 默认关闭；打开后模型多一个「代码索引」工具，用来找符号、依赖和修改前应检查的位置，再按建议去 `read` 源码
  - 索引在后台建立，不挡住聊天；空闲后自动释放解析进程，平时只保留本地查询
  - 设置里可开关、重建和打开索引目录；正常就绪时界面不出声，只在首次构建或异常时提示
  - 无界面评测可开对照臂比较探索次数、工具结果体积和压缩次数；任务或指标对不上时不会当成有效对照
  - 安装后可以建索引、查代码结构；关闭时聊天和原有工具不变
  - 语法资源单独放在安装目录，安装包不重复携带、也不带上无界面评测产物

## 2026-08-11

- **perf(agent)**: 编码 harness 可靠性升级：长会话缓存更稳、失败能换路、编辑落盘可核对
  - 大型工具结果归档占位符在同一任务内与多轮对话间保持稳定（按内容寻址），模型提示词缓存不再因随机标识漂移而失效，长会话后续请求更多命中缓存、更省 token
  - 同一调用连续失败达到阈值时，先引导模型换路（修前置条件、换参数或换工具），仍重复同一失败调用才中断；无关工具的成功不再错误清零失败计数
  - 编辑文件写入后重读真实字节逐字节核对，不一致时恢复原文并返回可诊断错误，杜绝静默错写
  - 模型网关返回带未转义控制字符的工具参数时受控修复后继续执行，不再整轮中断；修复次数进入事件与汇总，可观测
  - Bash 等工具子进程不再继承模型服务凭据，缩小密钥暴露面
  - 无界面任务：执行指令聚焦定位、实施、验证；仅产出思考而未给出正文或动作时不再被误判为任务完成
  - 测试：runtime/agent、artifacts、editTool、headless 等 59 个文件 676 条用例通过；typecheck 通过
- **fix(eval)**: DeepSWE 评测链路修复（仅评测基础设施，不影响产品行为）
  - 迁移到官方 Pier 0.3 任务生命周期，恢复官方补丁判分链路；评测网络严格隔离，仅对模型调用开放冻结入口，结果绑定真实模型端点
  - 修复 Pier 代理启动脚本在 Windows 下被写成 CRLF 导致代理容器启动即崩的问题（对固定版本打幂等 LF 补丁，冻结配置记录摘要并在执行前校验）
  - 零费用补丁 canary 改为与正式评测相同的代理路径，代理健康、补丁提取、官方判分全链路验证通过
  - 测试：Python harness 33 条用例通过
- **fix(headless)**: 无界面批处理任务结束后进程可靠退出
  - 任务里用 nohup 等方式拉起的后台进程会继承工具子进程的管道句柄，导致任务实际已完成、摘要已输出，进程却被残留句柄拖住不退出，批处理/评测等外部调用方只能干等到超时；现在摘要写完并真正刷入管道后立即退出，管道对端异常时 5 秒兜底强制退出
  - 代理出网链路支持 NOVA_PROXY_DEBUG=1 诊断开关，可按阶段（CONNECT 建连、TLS 握手、请求发出）排查企业代理或隔离网络问题，输出不含凭据
  - 测试：退出保证定向用例 3 条；typecheck、全量单测通过
- **feat(ui)**: 会话导航体验对齐主流 Agent 桌面端：双栏通顶布局 + 置顶分区 + 会话路径面包屑
  - 拆掉贯穿窗口的顶栏：侧边栏与内容区各自通顶、中间分界线分开；侧栏顶行放品牌与折叠开关，内容区顶行放路径面包屑与右侧操作组，两行各自承担本栏的窗口拖拽
  - 侧边栏新增置顶分区：会话菜单可把常用会话钉到顶部独立分区（带图钉标识），置顶状态随会话落盘，重开应用不丢；置顶只改展示分区，不打乱各项目的最近活跃排序
  - 内容区顶行靠分界线一侧新增「工作区 / 当前会话」无边框路径面包屑，点击展开当前工作区下的会话列表快速切换；焦点在子代理会话时显示其父会话，与侧边栏高亮规则一致
  - 内容区顶行右侧新增当前会话「⋯」菜单：置顶/取消置顶、删除当前会话，不用回侧边栏找行；窗口控制按钮保持右上角原位
  - 工程：置顶字段为可选持久化字段，旧会话缺省视为未置顶，无需迁移；置顶写入刻意不刷新会话活跃时间；侧边栏会话行渲染收敛为单一实现，置顶分区与项目分组共用
  - 测试：置顶持久化读写往返、摘要透传、不刷新活跃时间、服务广播链路、置顶投影纯函数等定向用例；typecheck / 架构门禁通过，全量测试除本机既有环境失败外全绿

## 2026-08-07

- **feat(compose)**: compose 模式重建为主 Agent 亲自带队的六阶段生命周期：阶段指南注入 + 按阶段工具门禁 + 顶部阶段条
  - 模式指令改写：主 Agent 按「构思 → 计划 → 开发 → 验证 → 审查 → 收尾」亲自推进，不再路由到编排工作流；每轮对话尾部注入当前阶段指南（六份 markdown，只写目标、完成标准与交接约定），切阶段只改尾部，历史消息与缓存前缀不动；废弃的编排入口在 compose 下物理拦截，防止误入旧流水线
  - 按阶段工具门禁：构思只读、计划仅可写计划文档，开发起放开完整工具集；越界调用返回中文可读原因并说明解锁方式；自动推进（auto）轮次确认门自动通过、不等用户回复，工具门禁仍按阶段生效
  - 聊天顶部新增常驻阶段条：六节点横排细线串联，当前阶段呼吸高亮、完成打勾、跳过带原因标记、待办置灰；点击已完成节点展开详情（计划节点可查看并打开计划文档）；「⋯」菜单提供手动完成/跳过/回退兜底（原因必填、运行中锁定），与生命周期工具共用同一套转换校验与唯一事件源；阶段转换调用不再以工具行刷屏
  - 会话恢复：会话详情透出阶段表，切会话即水合阶段条；旧会话无阶段表时按初始表纯显示，不回填落盘
  - 工程：阶段游标推导（当前阶段/终态/可回退范围）收敛为 shared 单一实现，门禁与 UI 投影共用；import 边界扫描器学会剥离 Vite `?raw` 查询后缀（指南以 `.md?raw` 注入）；新增 IPC 入口校验会话模式与类型，非 compose 主会话无法写入阶段表
  - 测试：门禁放行/拒绝矩阵、指南随阶段切换与 auto 行为、IPC 边界与事件推送、投影纯函数、阶段条交互等定向用例；typecheck / 架构门禁 / 全量测试通过（skillZip、SubagentSessionHeader 等 5 例为本机环境与分支既有失败，与本次改动无关）

- **feat(compose)**: 审查阶段改为唯一只读子代理独立审查；所有子代理在消息流统一为紧凑活动行，改动文件后附 diff 卡
  - 审查合同：主 Agent 用自然语言 brief（需求背景、计划位置、改动清单、验证证据）向内置 review 子代理交接，如同向同事交底；子代理只读独立审查正确性、范围、架构边界与安全，交回 markdown 报告；通过与否由主 Agent 阅读后亲自判断，不再解析结构化输出
  - 修复-复审循环上限兜底：审查阶段发出的回退计入会话级循环计数（随会话持久化，重开不丢），第 4 次回退被拒绝并提示主 Agent 向用户说明、停在审查阶段；回退计划等绕行路径同样计数，无法绕开上限无限重审
  - 子代理紧凑三段式：消息流中的子代理统一为一条紧凑活动行——状态点脉冲 + 名称 +（模型 · 思考强度）+ 状态与耗时；次行运行中显示当前动作（左→右呼吸微光，与阶段条同一动效语言），完成后翻转为结果一句话；点击在原处弹出悬浮面板查看工具调用记录、思考摘要与最终报告，运行中实时刷新，不再跳离当前会话
  - 子代理改动文件后行下方出现 diff 卡：N 个文件、逐文件 +/- 行数、超出折叠；Review 展开内联差异审查，逐文件接受/拒绝经路由表接回既有消息级检查点恢复；只读子代理无 diff 卡
  - 工程：子代理运行时继承父会话思考强度覆盖，投影显示与运行实际一致；新增会话级差异聚合（跨消息取每文件最早备份对比当前工作区）与对应 IPC；差异增删行数统计收敛为 shared 单一实现
  - 测试：循环上限与绕行拒绝、计数持久化恢复、会话级聚合（多消息合并/先建后删/审查痕迹合并/路由表）、三段式渲染与面板开关、diff 卡出现条件与审查路由等定向用例；typecheck / 架构门禁 / build 通过，全量测试除本机环境与分支既有失败外全绿
