...
export const savePlanTool: ToolExecutor = {
  name: 'save_plan',
  description:
    'Save the plan required by the current mode as a Markdown document inside the project. ' +
    'The path is fixed by the Runtime under .nova/plans/; callers cannot specify arbitrary paths. ' +
    'Reusing the same title in the same session revises the current plan; a new title generates a new readable filename.',
  ...
  parameters: {
    ...
    properties: {
      title: {
        type: 'string',
        description: 'A short, readable plan title; used as the Markdown filename.'
      },
      content: {
        type: 'string',
        description:
          'Markdown body. In compose mode, state what you want, what I decided not to do, and the technical choices; the capability list is generated by the capabilities parameter — no need to repeat it in the body. In plan mode, include goals, scope, architecture rationale, implementation steps, protected behavior, risks, verification, and rollback.'
      },
      capabilities: {
        type: 'array', minItems: 1, maxItems: 100,
        items: { type: 'string', minLength: 1, maxLength: 1000 },
        description: 'Required in compose mode: actionable, observable acceptance capabilities, one line of plain text per item; display titles and numbering are generated automatically. Not used in other modes.'
      }
    },
